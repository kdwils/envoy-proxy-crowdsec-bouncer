...
func TestServer_handleLiveness(t *testing.T) {
	...
	t.Run("returns 200 OK", func(t *testing.T) {
		...
		mockBouncer := mocks.NewMockBouncer(ctrl)
		s := NewServer(getDefaultConfig(), mockBouncer, nil, mocks.NewMockTemplateStore(ctrl), log)
...
func TestServer_handleReadiness(t *testing.T) {
	...
	t.Run("returns 500 when bouncer is nil", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()

		s := NewServer(getDefaultConfig(), nil, nil, mocks.NewMockTemplateStore(ctrl), log)
	...
	t.Run("returns 503 when bouncer is not ready", func(t *testing.T) {
		ctrl := gomock.NewController(t)
		defer ctrl.Finish()

		mockBouncer := mocks.NewMockBouncer(ctrl)
		mockBouncer.EXPECT().IsReady().Return(false)
	...
	t.Run("returns 200", func(t *testing.T) {
		...
		mockBouncer := mocks.NewMockBouncer(ctrl)
		mockBouncer.EXPECT().IsReady().Return(true)

		s := NewServer(cfg, mockBouncer, nil, mocks.NewMockTemplateStore(ctrl), log)
