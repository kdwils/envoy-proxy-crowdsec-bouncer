...
- **Never use inline constructors in tests** — always call the real constructor (`New...`/`New(...)`) when one exists; never hand-assemble the struct with a literal to skip the constructor's wiring, and never mutate a field on the result to substitute a mock or noop afterward. When a component (real, mock, or noop) needs to be substituted, pass it straight into the constructor as an argument — one call should produce the fully-formed, consistent object, so there's never a struct sitting around half from the constructor and half hand-patched. `bouncer.New` takes `DecisionCache`/`WAF`/`CaptchaService`/`*crowdsec.MetricsService` as arguments for exactly this reason; `bouncer.NewComponents` is the one place that resolves real-vs-noop from config for production and functional-test use.
