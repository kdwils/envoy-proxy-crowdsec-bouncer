...
- Defaults live in `cmd/root.go` as `viper.SetDefault` calls — when adding a config key, add the struct field in `config/` **and** the default in `cmd/root.go`, or the zero value silently applies.
- Env vars: prefix `ENVOY_BOUNCER_` with `.`→`_` mapping (e.g. `ENVOY_BOUNCER_BOUNCER_APIKEY`... verify exact key form against the replacer in `cmd/root.go`); `--config` flag accepts a yaml/json file.
