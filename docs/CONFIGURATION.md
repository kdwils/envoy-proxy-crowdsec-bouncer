...
| `failOpen` | bool | `false` | No | On WAF error (transport error or AppSec `error` action), allow the request and skip inspection instead of failing closed. IP-based LAPI decisions are still enforced |
