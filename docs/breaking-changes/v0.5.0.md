...
| v0.4.x | v0.5.0 (SecureCookie=false) | v0.5.0 (SecureCookie=true) |
|--------|-----------------------------|-----------------------------|
| `captcha_verified` | `session` | `__Host-session` |
...
If not set, defaults to `session` (or `__Host-session` when `secureCookie: true`).
